...
export default function RegisterPage() {
  ...
  const { register, handleSubmit, trigger, getValues, setValue, formState: { errors } } = useForm<RegisterFormData>({ resolver: zodResolver(registerSchema), mode: 'onChange', defaultValues: { subscriptionPlan: '' } });
  ...
  const handleContinue = async () => { if (plans.length === 0) { setError('Pricing packages are temporarily unavailable. Please try again shortly.'); return; } if (await trigger(['companyName', 'companyEmail', 'companyTin', 'companyPhone', 'subscriptionPlan'])) setStep(2); };
