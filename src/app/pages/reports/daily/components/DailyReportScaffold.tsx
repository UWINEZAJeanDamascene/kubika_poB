...
export function DailyReportScaffold({
  title,
  shortTitle,
  subtitle,
  icon: PageIcon,
        ...
        <div className="mx-auto max-w-7xl space-y-6">
            ...
            <div className="grid gap-6 p-5 lg:grid-cols-[minmax(0,1fr)_360px] lg:p-6">
              <div className="min-w-0">
                ...
                <div className="mt-4 flex items-start gap-3">
                  <div className={`rounded-xl p-3 ring-1 ${toneClass[tone].icon}`}>
              ...
              <Card className="border-slate-200 bg-slate-50 shadow-none dark:border-white/10 dark:bg-white/[0.04]">
                <CardContent className="space-y-4 p-4">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <Button variant="outline" size="sm" onClick={onDownloadPDF} disabled={downloading === "pdf"} className="border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950">
                      {downloading === "pdf" ? <Loader2 className="mr-1.5 h-4 w-4 animate-spin" /> : <Download className="mr-1.5 h-4 w-4" />}
                      PDF
                    </Button>
                    <Button variant="outline" size="sm" onClick={onDownloadExcel} disabled={downloading === "excel"} className="border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950">
                      {downloading === "excel" ? <Loader2 className="mr-1.5 h-4 w-4 animate-spin" /> : <Download className="mr-1.5 h-4 w-4" />}
                      Excel
                    </Button>
                  </div>
                </CardContent>
          ...
          {metrics.length > 0 && (
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
              {metrics.map((metric) => {
                ...
                return (
                  <Card key={metric.label} className="overflow-hidden border-slate-200/80 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-950">
                    <CardContent className="p-5">
                      <div className="flex items-start justify-between gap-4">
                        <div className="min-w-0">
                          ...
                          <div className="mt-3 truncate text-2xl font-bold tracking-tight text-slate-950 dark:text-white">{metric.value}</div>
                        </div>
                        <div className={`rounded-lg p-2.5 ring-1 ${toneClass[metric.tone].icon}`}>
                          <Icon className="h-5 w-5" />
                        </div>
                      </div>
                      {metric.caption && <p className="mt-3 truncate text-xs text-slate-500 dark:text-slate-400">{metric.caption}</p>}
                    </CardContent>
                  </Card>
                );
              })}
          ...
          <div className="rounded-xl border border-slate-200 bg-white px-5 py-4 text-sm text-slate-500 shadow-sm dark:border-slate-800 dark:bg-slate-950 dark:text-slate-400">
            <div className="flex items-start gap-3">
              ...
              <p>Daily report data is company scoped, permission controlled, and generated for the selected operating date.</p>
            </div>
          </div>
        </div>
      </div>
